$(document).ready(async function () {
  ...
  function getGenres() {
    return new Promise((resolve) => {
      $.get(
        "https://api.themoviedb.org/3/genre/movie/list?api_key=3699bcfd1aa3d5642b631dafd0a6d76e&language=en-US"
      ).then((genres) => {
  ...
  function getRecommended(type, id) {
    return new Promise((resolve) => {
      $.get(
        `https://api.themoviedb.org/3/${type}/${id}/recommendations?api_key=3699bcfd1aa3d5642b631dafd0a6d76e&language=en-US&page=1`
      ).then((results) => {
        let recommendedHtml = `<h2 class="text-center w-100 mt-5">Recommended Titles</h2>`;
        for (let i = 0; i < 8; i++) {
          recommendedHtml += `<div class="card-body col-3 p-0 ">
